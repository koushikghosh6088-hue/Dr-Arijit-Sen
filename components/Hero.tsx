    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          {/* Image & Quote Section */}
          <div className="relative mt-8 sm:mt-12 lg:mt-0 px-2 sm:px-0">
            <motion.div
              initial={{ opacity: 0, scale: 0.8, rotate: -2 }}
              animate={{ opacity: 1, scale: 1, rotate: 0 }}
              transition={{ duration: 1.2, ease: [0.16, 1, 0.3, 1] }}
              style={{ scale, rotateY: useTransform(scrollYProgress, [0, 1], [0, 15]) }}
              className="relative z-10 rounded-[2rem] sm:rounded-[3rem] overflow-hidden border-2 sm:border-8 border-blue-800/50 dark:border-blue-900/30 shadow-2xl"
            >
              <img
                src="/hero-doctor.jpg"
            ...
            <motion.div
              ...
              className="absolute -bottom-10 sm:-bottom-8 -left-1 sm:-left-6 lg:-left-12 bg-white/10 dark:bg-slate-900/40 backdrop-blur-2xl p-5 sm:p-8 rounded-2xl sm:rounded-[3rem] border border-white/20 dark:border-slate-800 max-w-[240px] sm:max-w-sm shadow-2xl z-20 transition-all duration-300"
            >
              <Quote className="w-6 h-6 sm:w-10 sm:h-10 text-[#d9f99d] mb-3 sm:mb-4 opacity-50" />
              <p className="text-white text-xs sm:text-lg font-medium italic leading-relaxed mb-4 sm:mb-6">
                "My mission is to treat every patient with care, respect, and the highest standard of medical knowledge."
              </p>
              <div className="flex items-center gap-3 sm:gap-4">
                <div className="w-8 h-8 sm:w-12 sm:h-12 bg-gradient-to-br from-blue-400 to-blue-600 rounded-full flex items-center justify-center font-bold text-white shadow-lg border-2 border-white/20 text-[10px] sm:text-sm">
                  AS
                </div>
                <div>
                  <h4 className="font-bold text-white text-[10px] sm:text-sm">Dr. Arijit Sen</h4>
                  <p className="text-[8px] sm:text-xs font-bold text-[#d9f99d] uppercase tracking-widest">MBBS Hons</p>
                </div>
              </div>
            </motion.div>

            {/* Social Floating - Hidden on mobile */}
            <motion.div
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ delay: 1 }}
              className="hidden sm:flex absolute top-1/2 -right-4 lg:-right-8 flex-col gap-4 z-20"
            >
              {['f', 't', 'y', 'i'].map((s, i) => (
                <motion.div
                  key={i}
